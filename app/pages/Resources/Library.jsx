...
class Library extends Component {
	...
	formatRegions = array => array.reduce((str, item, i) => {
		if (!i) str += item;
		else str += `, ${item}`;
		return str;
	}, "")
	...
	getUniqueCountries = (d, dict, key) => {
		if (d) {
			const result = [];
			const array = [...new Set(d.map(f => f[`${key}`]))].filter(f => f != "-");
			const sorted = array.sort((a, b) => a.localeCompare(b));
			sorted.forEach(f => {
				const row = {};
				row["Region"] = dict[f]["Continent"];
				row["Country"] = f;
				result.push(row);
			});
			return result;
	...
	numberOfPapers = (d, key) => d.reduce((acc, it) => {
	...
	getGeomapData = (d, dict) => {
		if (d) {
			...
			const uniqueCountriesArray = this.getUnique(cleanDatabyCountry, "Country").filter(f => f != "-");
			const uniqueCountries = this.getUniqueCountries(cleanDatabyCountry, dict, "Country");
			...
			const countries = this.groupData(cleanDatabyCountry, uniqueCountriesArray, "Country", dict);
	...
	filterDatabyGeo = (data, filter, key) => data.filter(f => {
	...
	filterData = () => {
		const {data, filterRegion, filterCountry, filterSubtopics, uniqueCountries} = this.state;
		...
		let newFilterCountry = null;
		if (uniqueCountries) {
			if (filterRegion !== ' ') {
				newFilterCountry = [...new Set(uniqueCountries.filter(f => f.Region === filterRegion).map(m => m.Country))];
				newFilterCountry.unshift(' ');
			} else {
				newFilterCountry = [...new Set(uniqueCountries.map(m => m.Country))];
				newFilterCountry.unshift(' ');
			}
		}
		...
		if (filterSubtopics.length > 0) {
			...
			return [_filteredData.flat(), newFilterCountry];
		}
		else {
			return [_filteredDatabyGeo, newFilterCountry];
		}
	};
	...
	render() {
		...
		const countriesFilter = filterData[1];
